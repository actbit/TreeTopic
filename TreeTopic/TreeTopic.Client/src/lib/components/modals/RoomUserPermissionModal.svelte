<script lang="ts">
  ...
  async function loadData() {
    try {
      ...
      // Fetch available room permissions
      ...
      const permPromises = roomUsers.map(async (user) => {
        try {
          const perms = await api.get<any>(`/${tenant}/api/roomusers/${user.id}/permissions`);
  ...
  async function addPermissionToUser() {
    ...
    try {
      await api.post(`/${tenant}/api/roomusers/${selectedUserId}/permissions`, {
  ...
  async function removeAllUserPermissions(userId: string) {
    if (!confirm('Are you sure you want to remove all permissions for this user?')) return;
...
<Modal {isOpen} title="Room User Permission Management" onClose={handleClose} size="large" closeButton={!isLoading}>
    ...
    <div class="flex-1 overflow-auto p-6 space-y-6">
      ...
      {:else}
        ...
        <div class="flex justify-between items-center">
          <h3 class="text-lg font-semibold text-text">User Room Permissions</h3>
          <button
        ...
        {#if showAddUser}
          <div class="border border-border rounded-lg p-4 bg-surface">
            ...
            <div class="space-y-3">
              <div>
                <label for="user-select-room" class="block text-sm font-medium text-text mb-1">User</label>
                <select
                  id="user-select-room"
              ...
              <div>
                <label for="permission-select-room" class="block text-sm font-medium text-text mb-1">Permission</label>
                <select
                  id="permission-select-room"
          ...
          <div class="divide-y divide-border">
            {#each roomUsers as user}
              ...
              <div class="p-4">
                ...
                <div class="space-y-2">
                  <div>
                    <p class="text-xs text-text-light mb-2">Individual Permissions (in addition to role permissions)</p>
        ...
        <div class="border border-border rounded-lg overflow-hidden">
          <div class="bg-surface p-4 border-b border-border">
            <h3 class="font-semibold text-text">Room Permission List</h3>
          </div>
